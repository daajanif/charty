...
class AddSubscriptionReminderScreenState extends State<AddSubscriptionReminderScreen> {
  ...
  Future<void> init() async {
    ...
    if (mIsUpdate) {
      ...
      if (widget.subscriptionModel.dueDate != null) {
        mRecurring = false;
        expiryDateController.text = DateFormat('yyyy-MM-dd').format(widget.subscriptionModel.dueDate);
      } else {
        mRecurring = true;
        firstPaymentController.text = DateFormat('yyyy-MM-dd').format(widget.subscriptionModel.firstPayDate);
  ...
  Future<void> showDateFrom() async {
    ...
    if (date != null) {
      if (mRecurring) {
        firstPaymentController.text = DateFormat('yyyy-MM-dd').format(date);
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        titleSpacing: 0,
        title: Text('Add Subscription', style: primaryTextStyle()),
        actions: [
          mIsUpdate
              ? TextButton(
                  onPressed: () {
                    ...
                    children: [
                      ...
                      mRecurring
                          ? Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text('Billing period', style: boldTextStyle()),
                                8.height,
                                Row(
                                ...
                                8.height,
                                Text('First payment', style: boldTextStyle()),
                                8.height,
                                AppTextField(
                                  onTap: () {
                                    showDateFrom();
                                  ...
                                  cursorColor: blueButtonColor,
                                  textFieldType: TextFieldType.NAME,
                                  decoration: subscriptionInputDecoration(name: 'e.g. yyyy-mm-dd'),
                                ).cornerRadiusWithClipRRect(defaultRadius),
                              ],
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(color: reminderColor, border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(defaultRadius)),
                            child: Text('Select Color', style: primaryTextStyle(color: reminderColor.isDark() ? Colors.white.withOpacity(0.85) : Colors.black), textAlign: TextAlign.center),
                          ).onTap(() {
                            selectColorDialog();
                          }),
                          16.height,
