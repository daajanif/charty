...
import 'package:mighty_notes/screens/SubscriptionDetailScreen.dart';
...
class SubscriptionReminderListScreenState extends State<SubscriptionReminderListScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        titleSpacing: 0,
        title: Text('Subscription Reminder', style: primaryTextStyle()),
      ),
                        ...
                        Row(
                          ...
                          children: [
                            Column(
                              ...
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(data.name.validate(), style: boldTextStyle(size: 20, color: getColorFromHex(data.color).isDark() ? Colors.white.withOpacity(0.85) : Colors.black), maxLines: 1, overflow: TextOverflow.ellipsis),
                                16.width,
                                Text(data.description.validate(), style: primaryTextStyle(color: getColorFromHex(data.color).isDark() ? Colors.white.withOpacity(0.85) : Colors.black), maxLines: 1, overflow: TextOverflow.ellipsis),
                              ],
                            ).expand(),
  ...
  Future<void> checkRecurringSubDate({SubscriptionModel subModel}) async {
    ...
    subscriptionService.updateDocument({'nextPayDate': subModel.nextPayDate}, subModel.id).then((value) {
      //
    }).catchError((error) {
